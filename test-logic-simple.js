...
async function loadQuestions() {
    try {
        ...
        alert('Błąd ładowania pytań. Odśwież stronę.');
        return false;
    }
}
...
function showResults(results) {
    ...
    if (results.passed) {
        resultMessage.innerHTML = `
            <div style="color: #4CAF50;">✅ Gratulacje! Test zaliczony!</div>
            <p style="font-size: 1rem; color: #666; margin-top: 10px;">
                Możesz przejść do praktycznej kalibracji case studies.
            </p>
        `;
    } else {
        resultMessage.innerHTML = `
            <div style="color: #F44336;">❌ Test niezaliczony</div>
            <p style="font-size: 1rem; color: #666; margin-top: 10px;">
                Wymagane minimum: 80%. Przejrzyj odpowiedzi i spróbuj ponownie.
            </p>
        `;
    }
    ...
    let reviewHTML = '<h3 style="margin-bottom: 20px;">📋 Przegląd odpowiedzi:</h3>';
        ...
        reviewHTML += `
            <div class="review-item ${detail.isCorrect ? 'correct' : 'incorrect'}">
                <div style="font-weight: 700; margin-bottom: 10px;">
                    ${detail.isCorrect ? '✅' : '❌'} Pytanie ${index + 1}: ${question.text}
                </div>
                <div style="margin-bottom: 8px;">
                    <strong>Twoja odpowiedź:</strong> ${userAnswerText}
                </div>
                ${!detail.isCorrect ? `
                    <div style="margin-bottom: 8px; color: #4CAF50;">
                        <strong>Prawidłowa odpowiedź:</strong> ${correctAnswerText}
                    </div>
                ` : ''}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🎯 Test wiedzy - gotowy do startu');
});
